...
ALERT_MANAGER_DIR=$(echo $PWD|sed 's/dockerfiles/alert_manager/')
printf "  volumes:\n" >> fig.yml
printf "    - $ALERT_MANAGER_DIR:/srv/www/alert_manager\n" >> fig.yml 

# Use a docker config.ini leaving the config.py system in place for staging/prod scripting
cp alert_manager_web/config.ini $ALERT_MANAGER_DIR/config.ini
